	...
	class API extends REST {
		...
		*/
		...
		private function customers(){	
			...
			$query="SELECT distinct c.id, c.customerName, c.email,c.country FROM angularcode_customers c order by c.id desc";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function customer(){	
			...
			if($id > 0){	
				$query="SELECT distinct c.id, c.customerName, c.email, c.country FROM angularcode_customers c where c.id=$id";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
					$result = $r->fetch_assoc();	
					$this->response($this->json($result), 200); // send user details
		...
		private function insertCustomer(){
			...
			$column_names = array('customerName', 'email', 'country');
		...
		private function updateCustomer(){
			...
			$query = "UPDATE angularcode_customers SET ".trim($columns,',')." WHERE id=$id";
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function deleteCustomer(){
			...
			$id = (int)$this->_request['id'];
			if($id > 0){				
				$query="DELETE FROM angularcode_customers WHERE id = $id";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
